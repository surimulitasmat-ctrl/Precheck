/* =========================
   ...
   Matches your latest index.html IDs:
   - #sessionLine (topbar session badges)
   - #bottomNav with: #navHome #navAlerts #navManager #navLogout
   - #modalBackdrop #modalTitle #modalBody #modalClose
   - #toast
   ========================= */
...
(() => {
  ...
  // Optional forced manual date-only items:
  ...
  // Default tile styles for known categories (fallback used for anything new)
  const TILE_META = {
    ...
    // If you add new categories (e.g. Fountain Drinks), they will use fallback icon/tone.
  };
  ...
  // Optional topbar buttons (we keep hidden; bottom nav is primary)
  ...
  const state = {
    ...
    categories: [...DEFAULT_CATEGORIES], // fetched per store if server supports
    items: [],
  ...
  /* ---------- Fetch categories/items (per store) ---------- */
  async function loadCategories() {
    // If server supports store categories endpoint, use it.
    // Otherwise fallback to DEFAULT_CATEGORIES.
    ...
    try {
      ...
      // Expect rows like: [{ name, sort_order, is_active, deleted_at }]
      const cats = (rows || [])
        .filter((c) => c && (c.is_active ?? true) && !c.deleted_at)
  ...
  async function loadItems() {
    ...
    // Expect: [{id,name,category,sub_category,shelf_life_days,is_active,deleted_at}]
    state.items = (rows || [])
      .filter((x) => x && (x.is_active ?? true) && !x.deleted_at)
      .map((x) => ({
        ...x,
  ...
  function categoryCounts() {
    ...
    for (const it of state.items) {
      ...
      if (counts[c] == null) counts[c] = 0; // if item has category not in list
      counts[c]++;
    }
    return counts;
  }
  ...
  function updateSessionLine() {
    ...
    const line = `${escapeHtml(store)} • ${escapeHtml(shift)} • ${escapeHtml(staff)}`;

    // IMPORTANT: only ONE badge shown (fixes "MANAGER + STAFF" showing together)
    sessionLine.innerHTML = `
      <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
        ${roleBadge}
        <span style="font-weight:1000;opacity:.95">${line}</span>
      </div>
    `;
  ...
  function hideTopbarButtons() {
    if (btnManagerTop) btnManagerTop.classList.add("hidden");
    if (btnLogoutTop) btnLogoutTop.classList.add("hidden");
  }

  function setActiveTab(el, on) {
    if (!el) return;
  ...
  function updateBottomNav() {
    ...
    if (nav.root) nav.root.classList.toggle("hidden", !hasSession);

    const page = state.view.page;
    setActiveTab(nav.home, page === "home" || page === "category" || page === "sauce_menu" || page === "session");
    setActiveTab(nav.alerts, page === "alerts");
    setActiveTab(nav.manager, page === "manager");
  }

  function bindBottomNav() {
    // Guard: if IDs missing, do nothing (prevents dataset null crash)
    if (!nav.home || !nav.alerts || !nav.manager || !nav.logout) return;

    // Bind once
    if (nav.home.dataset.bound === "1") return;
    nav.home.dataset.bound = "1";
    nav.alerts.dataset.bound = "1";
    nav.manager.dataset.bound = "1";
    nav.logout.dataset.bound = "1";
    ...
    nav.manager.addEventListener("click", () => {
  ...
  function renderSession() {
    ...
    const storeOptions = DEFAULT_STORES.map((s) => `<option value="${s}">${s}</option>`).join("");
            ...
            .map((cat, idx) => {
              ...
              return `
                <button class="tile tile--${meta.tone}" style="animation-delay:${delay}s" data-cat="${escapeHtml(cat)}" type="button">
                  <div class="tile-top">
                    <div class="tile-icon" aria-hidden="true">${meta.icon || ICONS.default}</div>
                  </div>
  ...
  function openManagerLogin() {
    ...
    btn.addEventListener("click", async () => {
      ...
      try {
        // Store-aware manager login (recommended)
  ...
  /* ---------- Manager page (items only UI here) ---------- */
  async function renderManager() {
    ...
    main.innerHTML = `
      <div class="card">
        <div class="h1">Manager</div>
        <div class="muted">
          Store: <strong>${escapeHtml(state.session.store)}</strong>
          <div style="margin-top:8px;">
            This page edits <strong>items</strong> for the selected store.
          </div>
        </div>
      </div>
    ...
    let rows = [];
    try {
      // Store-aware manager items (recommended)
            ...
            <div style="border-top:1px dashed rgba(0,0,0,0.10);padding-top:12px;margin-top:12px;">
              ...
              <div class="field">
                ...
                <select class="input mgr-cat" data-id="${r.id}">
                  ${cats
                    .map((c) => `<option value="${escapeHtml(c)}" ${norm(c) === norm(cat) ? "selected" : ""}>${escapeHtml(c)}</option>`)
                    .join("")}
                </select>
              </div>
  ...
  function openManagerAddItem() {
    ...
    openModal(
      "Add Item",
      `
      <div class="modal-item-title">New item (${escapeHtml(state.session.store)})</div>
